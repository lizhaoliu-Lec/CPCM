...
if IS_OLD_ME:
    from MinkowskiEngine.Common import RegionType, KernelGenerator
else:
...
from MinkowskiEngine.MinkowskiNormalization import MinkowskiBatchNorm, MinkowskiInstanceNorm
from MinkowskiEngine.MinkowskiPooling import MinkowskiAvgPooling, MinkowskiSumPooling
...
def get_norm(norm_type, n_channels, D, bn_momentum=0.1):
    if norm_type == NormType.BATCH_NORM:
        return MinkowskiBatchNorm(n_channels, momentum=bn_momentum)
    elif norm_type == NormType.INSTANCE_NORM:
        return MinkowskiInstanceNorm(n_channels)
    elif norm_type == NormType.INSTANCE_BATCH_NORM:
        return nn.Sequential(
            MinkowskiInstanceNorm(n_channels),
...
conv_to_region_type = {
    # kernel_size = [k, k, k, 1]
    ConvType.HYPERCUBE: RegionType.HYPERCUBE if IS_OLD_ME else RegionType.HYPER_CUBE,
    ConvType.SPATIAL_HYPERCUBE: RegionType.HYPERCUBE if IS_OLD_ME else RegionType.HYPER_CUBE,
    ConvType.SPATIO_TEMPORAL_HYPERCUBE: RegionType.HYPERCUBE if IS_OLD_ME else RegionType.HYPER_CUBE,
    ConvType.HYPERCROSS: RegionType.HYPERCROSS if IS_OLD_ME else RegionType.HYPER_CROSS,
    ConvType.SPATIAL_HYPERCROSS: RegionType.HYPERCROSS if IS_OLD_ME else RegionType.HYPER_CROSS,
    ConvType.SPATIO_TEMPORAL_HYPERCROSS: RegionType.HYPERCROSS if IS_OLD_ME else RegionType.HYPER_CROSS,
    ConvType.SPATIAL_HYPERCUBE_TEMPORAL_HYPERCROSS: RegionType.HYBRID if IS_OLD_ME else RegionType.HYPER_CUBE
}

if IS_OLD_ME:
    int_to_region_type = {m.value: m for m in RegionType}
else:
    int_to_region_type = {m.value: m for m in [
        RegionType.CUSTOM,
        RegionType.HYPER_CROSS,
...
def convert_conv_type(conv_type, kernel_size, D):
    ...
    if conv_type == ConvType.SPATIAL_HYPERCUBE:
        ...
        if IS_OLD_ME:
            axis_types = [
                             RegionType.HYPERCUBE if IS_OLD_ME else RegionType.HYPER_CUBE,
                         ] * 3
            if D == 4:
                axis_types.append(RegionType.HYPERCROSS if IS_OLD_ME else RegionType.HYPER_CROSS)
        else:
            if D == 4:
                raise ValueError("D == 4 require RegionType.HYBRID, which is deprecated in version later 0.4.3")
            pass
...
def conv_tr(in_planes,
    ...
    if IS_OLD_ME:
        return MinkowskiConvolutionTranspose(
            in_channels=in_planes,
            out_channels=out_planes,
            kernel_size=kernel_size,
...
def avg_unpool(kernel_size, stride=1, dilation=1, conv_type=ConvType.HYPERCUBE, D=-1):
    ...
    if IS_OLD_ME:
        ...
        from MinkowskiEngine.MinkowskiPooling import MinkowskiPoolingTranspose
        return MinkowskiPoolingTranspose(
            kernel_size=kernel_size,
            stride=stride,
            dilation=dilation,
            kernel_generator=kernel_generator,
